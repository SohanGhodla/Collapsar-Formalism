...
def find_closest_r_in(M_, a_, ut, r_min, r_max,  max_iter=500):
    iter_count = 0
    X = ut
    tol=1e-5 * X
...
while t < tmax:
    ...
    if Rin > Rd:
        Rin = Rd
    Rt = max(Rin, min(Rd, (2*Rg/Rd**s_PL * Mdotacc/msun * 10**2.5)**(1./(1-s_PL))))
    ...
    if Rt > Rin:
        Jbhdot =  (0.86 * Mbh * G / c) * Macc   - P_BZ / (k*Omega_H)
        Jnutrino_dot =  u_phi(M_, Rt, a_) * Mdotacc * (Rt/Rd)**s_PL  
    else:
        # Jbhdot =  u_phi(M_, Rt, a_) * Macc  - P_BZ / (k*Omega_H)
        Jbhdot =  (0.86 * Mbh * G / c) * Macc   - P_BZ / (k*Omega_H)
        Jnutrino_dot =  (0.86 * Mbh * G / c)  * Macc 
    ...
    #below 0.05 is to account for neutrino loss in the disk
    R_avg = 2.1*Risco   # to account for gravitional redshift
    gtt_fac_2 =  np.sqrt(1 - 2*M_/R_avg)

    Mbhdot =  0.97 * Mdotacc * (Rt/Rd)**s_PL  -    P_BZ / c**2#(1 - sqrt(2*Rg/(3*Risco))) * Macc 
    eta_acc = 0.03
    #0.01 factor to account for the rest lost in neutrinos
    Lacc = 0.01*eta_acc * Mdotacc * (Rt/Rd)**s_PL * c**2* gtt_fac_2 + P_BZ * gtt_fac 
    Lj = eta_acc * Mdotacc * (Rt/Rd)**s_PL * c**2* gtt_fac_2 + P_BZ * gtt_fac 
    
    Mfbdot = MJfbdot(t, tgrid, lgMfbdotgrid)
    Jfbdot = MJfbdot(t, tgrid, lgJfbdotgrid)

    Mddot = Mfbdot - Mdotacc
    Jddot = Jfbdot - Jwdot - (0.86 * Mbh * G / c) * Macc #- Jnutrino_dot 
...
file = open('./output/nonrel_MAD', 'wb')
import pickle
