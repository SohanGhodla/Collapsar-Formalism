# --------- This notebook assumes a Novikov-Thorne disk accretion flow -----------
...
def MJfbdot(t, tgrid, lgMJdot_grid):  # M/J fallback rate at a given time t
...
Jbh = Jbh0
abh = c*Jbh/(G*Mbh**2)
# ------ Do this to respect cosmic censorship during the initial 1msun collapse bit  --------
if abh >= 0.9994:  
    abh = 0.9994
    Jbh = abh * (G*Mbh**2)/c
...
def find_closest_r(M_, a_, Md, Jd, r_min, r_max,  max_iter=500):
    ...
    # If the bisection method doesn't converge, raise an exception
...
while t < tmax:
    ...
    Rt = max(Risco, min(Rd, (2*Rg/Rd**s_PL * Md / tvis / msun * 10**2.5)**(1./(1-s_PL)))) 
    ...
    Mbhdot =  u_t(M_, Risco, a_) * Mdotacc * (Rt/Rd)**s_PL / fac1  
    ...
    Mdot_lost = Mdotacc * (Rt/Rd)**s_PL  +  (result * Md * s_PL / tvis / Rd**s_PL)  # The first term is the mass that gets accreted or converted into neutrinos while the
                                                                                        # second is blown away in winds
    ...
    # --- ends up in the hole ----
    Jbhdot =  u_phi(M_, Risco, a_) * Mdotacc * (Rt/Rd)**s_PL / fac1 
    Jnutrino_dot =  u_phi(M_, Rt, a_) * Mdotacc * (Rt/Rd)**s_PL / fac1 - Jbhdot 

    i_grid = calc_grid_pt(t, tgrid) - 1

    Rd_temp = rarr[i_grid]
    gamma = np.sqrt( (a_**2 * M_ + 2*a_*np.sqrt(M_)*Rd_temp**1.5 + Rd_temp**3) / (2*a_*np.sqrt(M_)*Rd_temp**1.5 - 3*M_*Rd_temp**2 + Rd_temp**3) )
    
    # gamma = np.sqrt( (a_**2 / 2 + Rd_temp**2) / (a_**2 /2 - 2*M_*Rd_temp + Rd_temp**2) ) # print('gamma = ', gamma)
    # fac1 = gamma**3 * (a_**2 / Rd_temp**2 - 6*M_ / Rd_temp + 3)/3 

    # # gamma_rarr = np.sqrt( (a_**2 * M_ + 2*a_*np.sqrt(M_)*Rd_temp**1.5 + Rd_temp**3) / (2*a_*np.sqrt(M_)*Rd_temp**1.5 - 3*M_*Rd_temp**2 + Rd_temp**3) )
    # fac2 = gamma**2 * ( (a_/Rd_temp)**4 + 6*(a_/Rd_temp)**2 + 5 + 8*M_*a_**2/Rd_temp**3  - 10*a_*M_/Rd_temp**3/Omgarr[i_disk]) / 5
    ...
    # Mfbdot = fac1 * MJfbdot(t, tgrid, lgMfbdotgrid)
    # Jfbdot = fac2 * MJfbdot(t, tgrid, lgJfbdotgrid)
    ...
    eta_acc = (1 -  u_t(M_, Risco, a_) ) * gtt_fac
    Lacc =  eta_acc * Mdotacc / fac1 * (Rt/Rd)**s_PL * c**2

    dt = dt_over_tvis * tvis / np.sqrt(-gtt_factor) 
    t += dt
    Md += Mddot * dt
    Jd += Jddot * dt
    Mbh += Mbhdot * dt
