...
! ***********************************************************************
      ...
      subroutine my_other_eval_fp_ft( &
         ...
         integer, intent(out) :: ierr
         type (star_info), pointer :: s
         real(dp) :: w2, w4,w6, lg_one_sub_w4, inv_A_omega
         integer :: j
...
!$OMP PARALLEL DO PRIVATE(j, inv_A_omega, w2, w4, w6, lg_one_sub_w4) SCHEDULE(guided)
            do j=1, s% nz
               ...
               inv_A_omega = 1d0/(1d0-0.1076d0*w4-0.2336d0*w6-0.5583d0*lg_one_sub_w4)
               !fits for fp, ft
               fp(j) = inv_A_omega*(1d0-two_thirds*w2-0.06837d0*w4-0.2495d0*w6)
               ft(j) = inv_A_omega*(1d0+0.2185d0*w4-0.1109d0*w6)
...
!$OMP END PARALLEL DO
         ...
         if (s% u_flag) then
            ...
            do j=1, s% nz
               if (s% q(j) > 0.999) then
                  ...
                  fp(j) = fp(j)*(1d0-(s% q(j)-0.998)/(0.001)) + 1d0*(s% q(j)-0.998)/(0.001)
                  ft(j) = ft(j)*(1d0-(s% q(j)-0.998)/(0.001)) + 1d0*(s% q(j)-0.998)/(0.001)
               end if
            end do
         end if
      ...
      integer function extras_check_model(id)
         ...
         !      if (abs(maxval(s% xh(s% i_u, :)) - maxval(s% xh_old(s% i_u, :)))> 2d7) then
         !         write(*,*) "retry because of large change in max velocity"
      ...
      integer function extras_start_step(id)
         ...
         write(*,*) "check log_power_nonnuc_neutrinos", log_power_nonnuc_neutrinos
      ...
      subroutine my_before_struct_burn_mix(id, dt, res)
         ...
         if(s% u_flag) then
            ...
            ! !avoid shock breakout nastyness, if velocity near the surface indicates
            ! !breakout is near, just artificially accelerate the outer layers
            !if (s% u_flag) then
            !   !if ((maxval(s% xh(s% i_u,:s% nz)) > s% xh(s% i_u,1)+5d7 .and. maxval(s% xh(s% i_u,:s% nz))>0d0) .or. s% L(1)/Lsun>1d10) then
            !   if (s% xh(s% i_u,50) > s% xh(s% i_u,1)+5d7 .and. s% xh(s% i_u,50)>0d0) then
            !      write(*,*) "Homogeneizing velocity of 50 outermost cells to this value", s% xh(s% i_u,50), s% xh(s% i_u,50)/1d5
            !      s% xh(s% i_u, :50) = s% xh(s% i_u,50)
            !      s% u(:50) = s% xh(s% i_u,:50)
            !   end if
            !   !! ignoring surface cells merge
            !   !   write(*,*) "ignoring surface cells merge below k=",  maxloc(s% xh(s% i_u,:s% nz),dim=1)+100
            !   !   s% merge_amr_k_for_ignore_surface_cells = maxloc(s% xh(s% i_u,:s% nz),dim=1)+100
            !   !   s% merge_amr_ignore_surface_cells = .true.
            !   !else
         ...
         if (s% lxtra(lx_using_bb_bcs)) then
            ...
            !s% scale_max_correction_for_negative_surf_lum = .false.
         ...
         write(*,*) "check limits", s% delta_lgL_phot_hard_limit, s% relax_hard_limits_after_retry
      ...
      integer function extras_finish_step(id)
         ...
         if (remove_extended_layers .and. s% u_flag &
            .and. s% log_surface_radius > 3.9d0) then
            do k = 1, s% nz
               if (log10(s% r(k)/Rsun) < 3.9d0) then
                  exit
               end if
            end do
            s% use_atm_PT_at_center_of_surface_cell = .true.
